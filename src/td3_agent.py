    ...
    def log_weights_and_gradients(self, writer, step: int):
        ...
        for name, param in self.model.actor.named_parameters():
            if param.requires_grad:
                writer.add_histogram(f'td3_weights/actor/{name}', param.data, step)
                if param.grad is not None:
                    writer.add_histogram(f'td3_gradients/actor/{name}', param.grad, step)
        
        # Log critic networks weights and gradients
        for name, param in self.model.critic.named_parameters():
            if param.requires_grad:
                writer.add_histogram(f'td3_weights/critic/{name}', param.data, step)
                if param.grad is not None:
                    writer.add_histogram(f'td3_gradients/critic/{name}', param.grad, step)
